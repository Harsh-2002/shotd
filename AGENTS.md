...
- Installation and updates run only for the logged-in GUI user. Never use `sudo`, a LaunchDaemon, or system-wide binary paths. The public installer may idempotently manage only shotd's user PATH and zsh completion entries.
...
- Watch and output directories must not overlap. Optional source replacement must preserve format, recheck the source fingerprint, and avoid watcher loops.
- Keep user-facing processing logs concise: one success line per capture and actionable errors only.
- Keep shell completion generated by the CLI; installer shell changes must remain minimal, user-scoped, and idempotent.
